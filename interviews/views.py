...
def index(request):
    ...
    for x in range (len(LinkData) - 1,-1, -1):
        if ("resource" in LinkData[x].get("href") and "Assignment" not in NameData[x]) or ("Placement Offers" in NameData[x]) or ("Opportunity Update" in NameData[x]): #and "Placement Offers" not in NameData[x]:
