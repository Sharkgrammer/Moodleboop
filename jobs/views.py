...
def index(request):
    ...
    for x in range (0, len(LinkData)):
        if "resource" not in LinkData[x].get("href") and "Assignment" not in NameData[x] and "Placement Offers" not in NameData[x]:
            #data+= ("<h3><b><a href='" + LinkData[x].get("href") + "'>" + NameData[x] + "</a></b></h3>")
            data = ("<h3 class='beside'><b>" + NameData[x] + "   " + "<a href='" + LinkData[x].get("href") + "' class='but'>Submit link</a></b></h3>")
