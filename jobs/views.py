...
def index(request):
    ...
    for x in range (0, len(LinkData)):
        if "resource" not in LinkData[x].get("href") and "Assignment" not in NameData[x] and "Placement Offer" not in NameData[x] and "Opportunity Update" not in NameData[x] and "keane_kelly_daniel" not in NameData[x]::
